// The code published JSON data to a MQTT server. I am using the following topics for the server:
// 1. Treasure/Goku
// 2. Treasure/switch
// 3. Treasure/tempHum
...
void reconnect() {
  ...
  while (!mqtt.connected()) {
    ...
    if (mqtt.connect(mac, mqtt_user, mqtt_password)) { //<<---using MAC as client ID, always unique!!!
      ...
      mqtt.subscribe("Treasure/+"); //we are subscribing to 'theTopic' and all subtopics below that topic
...
void loop() {
  ...
  if (millis() - timerOne > 10000) {
    ...
    int GokuIronyLevel = 9000;
    sprintf(message, "{\"Power level is over\":\"%d%\"}", GokuIronyLevel); // %d is used for an int
    mqtt.publish("Treasure/Goku", message);
    timerOne = millis();
  }
  ...
  if (millis() - timerTwo > 15000) {
    ...
    mqtt.publish("Treasure/tempHum", message);
    timerTwo = millis();
  }
  ...
  if (millis() - timerThree > 23000) {
    ...
    mqtt.publish("Treasure/switch", message);
    timerThree = millis();
  }
}//end Loop
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, "Treasure/Goku") == 0) {
    Serial.println("A message from Goku . . .");
  }

  else if (strcmp(topic, "Treasure/tempHum") == 0) {
    Serial.println("Some weather info has arrived . . .");
  }

  else if (strcmp(topic, "Treasure/switch") == 0) {
